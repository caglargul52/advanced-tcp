...
namespace AdvancedTCP
{
    public struct ClientInfo
    {
        ...
        public string ipAndRemoteEndPoint;
        public string pcName;
    }

    public delegate void ClientConnectedEventHandler(ClientInfo clientInfo);
    public delegate void ClientDisconnectedEventHandler(ClientInfo clientInfo);
    public delegate void ReceivedMessageFromClientEventHandler(ClientInfo clientInfo, string message);
    ...
    public class Server
    {
        ...
        public bool SendMessage(ClientInfo info, string message)
        {
            var resultBlocked = (from x in BlockedIpList where x == info.ipAddress select x).FirstOrDefault();

            if (resultBlocked != null) return false;
        ...
        private bool MessageReceived(string ipPort, byte[] data)
        {
            ...
            if (message != null)
            {
                ...
                if (message.Type == Type.ClientLogon)
                {
                    lock (_lockOfClientList)
                    {
                        _clientList.Add(client);

                        MessageModel acceptMessage = new MessageModel();
            ...
            while (true)
            {
                if (true)
                {
                    Parallel.ForEach(_clientList.ToList(), async client =>
                    {
                        ...
                        if (!status)
                        {
                            if (_server != null) _server.DisconnectClient(client.ipAndRemoteEndPoint);
    ...
    internal class Tcp_Server : IDisposable
    {
